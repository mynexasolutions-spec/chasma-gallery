...
const getOne = async (req, res, next) => {
  try {
    ...
    const valuesRes = await pool.query(
      'SELECT id, value FROM attribute_values WHERE attribute_id = $1 ORDER BY value',
      [req.params.id]
    );

    res.json({
      success: true,
      data: { ...attrRes.rows[0], values: valuesRes.rows },
    });
  } catch (err) { next(err); }
};
...
const create = async (req, res, next) => {
  try {
    ...
    const id = uuidv4();
    const { rows } = await pool.query(
      'INSERT INTO attributes (id, name, slug) VALUES ($1, $2, $3) RETURNING *',
      [id, name, slug]
    );
...
const update = async (req, res, next) => {
  try {
    const { name, slug } = req.body;
    const { rows } = await pool.query(
      'UPDATE attributes SET name = $1, slug = $2 WHERE id = $3 RETURNING *',
...
const addValue = async (req, res, next) => {
  try {
    ...
    const id = uuidv4();
    const { rows } = await pool.query(
      'INSERT INTO attribute_values (id, attribute_id, value) VALUES ($1, $2, $3) RETURNING *',
      [id, req.params.id, value]
    );
    res.status(201).json({ success: true, data: rows[0] });
  } catch (err) { next(err); }
};

// PUT /api/attributes/:attrId/values/:valueId
const updateValue = async (req, res, next) => {
  try {
    const { value } = req.body;
    const { rows } = await pool.query(
      'UPDATE attribute_values SET value = $1 WHERE id = $2 AND attribute_id = $3 RETURNING *',
      [value, req.params.valueId, req.params.attrId]
    );
...
module.exports = { getAll, getOne, create, update, remove, getValues, addValue, updateValue, removeValue };
